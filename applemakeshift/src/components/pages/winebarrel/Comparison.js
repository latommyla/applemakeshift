import React from 'react'

import Racingsitdown from '../../images/products/RacingSitDown.png'
import Racingstandup from '../../images/products/RacingStandUp.png'

export default function Comparison() {
    return (
        <div class="font-lato m-auto pb-10 sm:pb-20 items-center w-screen sm:max-w-screen-xl">
            <div class="m-auto pt-28 text-center">
                <p class="font-bold m-20 text-3xl md:text-5xl mt-2"> Which Arcade is right for you? </p>
            </div>
            <div class="p-2 md:p-0 md:w-4/12 place-items-center justify-center m-auto grid md:gap-64 grid-cols-1 md:grid-cols-3 text-center">
            <div class="pt-0 md:pt-16 md:w-52">
                    <img class="w-44 h-44 m-auto" src={ Racingsitdown } alt="Racing Sit Down Machine Series S and X" />
                    <p class="pt-10 text-2xl font-bold"> Series S and X </p>
                    <p class="mt-3 md:mt-8"> From $4499 </p>
                    <button class="bg-blue-500 hover:bg-blue-400 text-white py-1 px-3 rounded-full mt-2"> Buy </button>
                    <p class="mt-2 hover:underline text-blue-500"> Learn more </p>
                    <hr class="my-4 mx-auto h-1 bg-gray-200 rounded border-0 md:my-10"></hr>
                    <div class="mt-10">
                        <p class="text-2xl"> Unlimited </p>
                        <p class="text-sm"> Games </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-2xl"> 32" </p>
                        <p class="text-sm"> LCD </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-2xl"> 1 </p>
                        <p class="text-sm"> Players </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Full Forced Feedback </p>
                        <p class="text-sm"> Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Trackball </p>
                        <p class="text-sm"> Not Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Free Play </p>
                        <p class="text-sm"> Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Coin Operated </p>
                        <p class="text-sm"> Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> 3 Year </p>
                        <p class="text-sm"> Warranty </p>
                    </div>
                    <hr class="my-4 mx-auto h-1 bg-gray-200 rounded border-0 md:my-10"></hr>
                </div>
                <div class="pt-16 md:w-52">
                    <img class="w-44 h-44 m-auto" src={ Racingsitdown } alt="Racing Sit Down Machine" />
                    <p class="pt-10 text-2xl font-bold"> Racing Sit Down </p>
                    <p class="mt-3 md:mt-8"> From $3999 </p>
                    <button class="bg-blue-500 hover:bg-blue-400 text-white py-1 px-3 rounded-full mt-2"> Buy </button>
                    <p class="mt-2 hover:underline text-blue-500"> Learn more </p>
                    <hr class="my-4 mx-auto h-1 bg-gray-200 rounded border-0 md:my-10"></hr>
                    <div class="mt-10">
                        <p class="text-2xl"> 107 </p>
                        <p class="text-sm"> Games </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-2xl"> 32" </p>
                        <p class="text-sm"> LCD </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-2xl"> 1 </p>
                        <p class="text-sm"> Players </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Full Forced Feedback </p>
                        <p class="text-sm"> Not Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Trackball </p>
                        <p class="text-sm"> Not Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Free Play </p>
                        <p class="text-sm"> Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Coin Operated </p>
                        <p class="text-sm"> Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> 3 Year </p>
                        <p class="text-sm"> Warranty </p>
                    </div>
                    <hr class="my-4 mx-auto h-1 bg-gray-200 rounded border-0 md:my-10"></hr>
                </div>
                <div class="pt-16 md:w-52">
                    <img class="w-44 h-44 m-auto" src={ Racingstandup } alt="Racing Stand Up Machine" />
                    <p class="pt-10 text-2xl font-bold"> Racing Stand Up </p>
                    <p class="mt-3 md:mt-8"> From $3192 </p>
                    <button class="bg-blue-500 hover:bg-blue-400 text-white py-1 px-3 rounded-full mt-2"> Buy </button>
                    <p class="mt-2 hover:underline text-blue-500"> Learn more </p>
                    <hr class="my-4 mx-auto h-1 bg-gray-200 rounded border-0 md:my-10"></hr>
                    <div class="mt-10">
                        <p class="text-2xl"> 107 </p>
                        <p class="text-sm"> Games </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-2xl"> 32" </p>
                        <p class="text-sm"> LCD </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-2xl"> 1 </p>
                        <p class="text-sm"> Players </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Full Forced Feedback </p>
                        <p class="text-sm"> Not Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Trackball </p>
                        <p class="text-sm"> Not Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Free Play </p>
                        <p class="text-sm"> Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> Coin Operated </p>
                        <p class="text-sm"> Supported </p>
                    </div>
                    <div class="mt-10">
                        <p class="text-xl"> 3 Year </p>
                        <p class="text-sm"> Warranty </p>
                    </div>
                    <hr class="my-4 mx-auto h-1 bg-gray-200 rounded border-0 md:my-10"></hr>
                </div>
            </div>
        </div>
    );
}